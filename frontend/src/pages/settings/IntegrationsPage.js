...
const IntegrationsPage = () => {
    ...
    const handleConfigureSave = async (selectedIds) => {
    ...
    return (_jsxs("div", { className: "container mx-auto px-4 py-8", children: [_jsx("h1", { className: "text-2xl font-bold text-gray-900 mb-6", children: "Integrations" }), loading ? (_jsx("div", { className: "flex justify-center items-center py-12", children: _jsx("div", { className: "w-8 h-8 border-4 border-blue-600 border-t-transparent rounded-full animate-spin" }) })) : (_jsx("div", { className: "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6", children: PROVIDERS.map((provider) => {
                    ...
                    return (_jsx(IntegrationCard, { provider: provider, integration: integration, onConnect: handleConnect, onDisconnect: handleDisconnect, onSyncNow: handleSyncNow, onConfigure: handleConfigure, connectingProvider: connectingProvider, disconnectingProvider: disconnectingProvider, syncingProviders: syncingProviders }, provider));
                }) })), _jsx(ActivityLog, { activities: activities, page: activityPage, total: activityTotal, limit: 20, onPageChange: (page) => fetchActivity(page), loading: activityLoading }), connectingModalProvider && (_jsx(ConnectModal, { open: true, provider: connectingModalProvider, onConfirm: handleModalConfirm, onCancel: handleModalCancel, loading: connectingProvider === connectingModalProvider })), disconnectingModalProvider && (_jsx(DisconnectDialog, { open: true, provider: disconnectingModalProvider, onConfirm: handleDisconnectConfirm, onCancel: handleDisconnectCancel, loading: disconnectingProvider === disconnectingModalProvider })), configuringProvider && (_jsx(ConfigureDrawer, { open: true, provider: configuringProvider, onClose: handleConfigureClose, onSave: handleConfigureSave, onReconnect: () => setConnectingModalProvider(configuringProvider) })), toast && (_jsx(Toast, { message: toast.message, type: toast.type, onDismiss: () => setToast(null) }))] }));
};
